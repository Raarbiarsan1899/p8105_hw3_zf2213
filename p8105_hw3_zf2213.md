...
``` r
ny_noaa <- p8105.datasets::ny_noaa %>% separate(col = date, into = c("year", "month", "day"), sep = "-") %>% rename(prcp_01mm = prcp, snow_mm = snow, snwd_mm = snwd) %>% mutate(tmax = as.integer(tmax), tmin = as.integer(tmin))
```

*Q1. For snowfall, what are the most commonly observed values? Why?* The dataset has many missing values. Most part are zeros. Most area are so warm that no snow can form.
...
``` r
ny_noaa %>% filter(month %in% c("01", "07")) %>% group_by(year, month) %>% summarize(mean_temp = mean(tmax, na.rm = TRUE)) %>% ggplot(aes(y = mean_temp, x = year)) + geom_point() + facet_grid(. ~ month)
```

![](p8105_hw3_zf2213_files/figure-markdown_github/unnamed-chunk-12-1.png)
...
![](p8105_hw3_zf2213_files/figure-markdown_github/unnamed-chunk-13-1.png)
